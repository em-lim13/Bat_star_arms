...
morphology_data1 <- read_csv("Data/morphology_data2.csv")
...
morphology_data <- morphology_data1 %>% 
  ...
  rename(site = Location, 
         ...
         longest_arm = Longest.Arm, # I double checked this is correct
...
table_a <- species_arm_data %>%
  ...
  mutate(total = sum(n)) %>%
  ungroup() %>%
  mutate(percent_atypical = round((100 - (100*n/total)), 2)) %>%
  filter(arm_type == "normal") 

# make the bulk of the table
table_b <- table_a %>%
  unite("percent_n", percent_atypical:total, remove = FALSE, sep = ", n = ") %>%
  dplyr::select(c(site, species, percent_n)) %>%
  pivot_wider(names_from = species, values_from = percent_n) %>%
  as.data.frame()

# just get site means
table_c <- table_a %>%
  dplyr::select(c(site, species, percent_atypical)) %>%
  pivot_wider(names_from = species, values_from = percent_atypical) %>%
  rowwise() %>%
  mutate(site_mean = round(mean(c_across(2:10), na.rm = TRUE), 2),
         site_sd = round(sd(c_across(2:10), na.rm = TRUE), 2)) %>%
  ungroup() %>%
  unite("site_mean_sd", site_mean:site_sd, remove = FALSE, sep = " ± ") %>%
  dplyr::select(site, site_mean_sd) %>%
  as.data.frame()

# species means
table_d <- table_a %>%
  dplyr::select(c(site, species, percent_atypical)) %>%
  pivot_wider(names_from = site, values_from = percent_atypical) %>%
  rowwise() %>%
  mutate(species_mean = round(mean(c_across(2:17), na.rm = TRUE), 2),
         species_sd = round(sd(c_across(2:17), na.rm = TRUE), 2)) %>%
  ungroup() %>%
  unite("species_mean_sd", species_mean:species_sd, remove = FALSE, sep = " ± ") %>%
  dplyr::select(species, species_mean_sd) %>%
  pivot_wider(names_from = species, values_from = species_mean_sd) %>%
  mutate(site = "Species mean ± sd") %>%
...
table_final <- table_b %>%
  rbind(table_d) %>%
  left_join(table_c, by = "site") %>%
  rename(`Patiria miniata` = bat,
         `Dermasterias imbricata` = leather,
         `Pisaster ochraceus` = ochre,
         `Orthasterias koehleri` = painted,
         `Henricia sp.` = blood,
         `Evasterias troschelii` = mottled,
         `Mediaster aequalis` = vermillion,
         `Stylasterias forreri` = velcro,
...
all_stars_arm_site_glm <- glmmTMB(arm_bin ~ site, 
                         family = binomial(link = 'logit'),
                         data = species_arm_data)
...
bat_arm_site_glm <- glmmTMB(arm_bin ~ site, 
                                  family = binomial(link = 'logit'),
                                  data = bat_arm_data)
...
colour_table <- colour_data %>%
  group_by(colour, arm_type) %>%
  count() %>%
  left_join(colour_data %>%
              group_by(colour) %>%
              count(), by = "colour") %>%
...
# check residuals
plot(simulateResiduals(oral_sa_lm))
...
# center depth
feeding_glm <- glmmTMB(feed_bin ~ arm_bin*substrate + depth_centre + (1|site), 
                            family = binomial(link = 'logit'),
                            data = feeding_data)
...
# What proportion were feeding?
sum_stats_feeding <- ggpredict(feeding_glm, terms = c("arm_bin", "substrate")) %>% 
  dplyr::rename(arm_bin = x,
...
# gamma
righting_time_gamma <- glmmTMB(time_total_s ~ arm_bin*size_centre + (1|Site),
...
AIC(righting_time_gaus, righting_time_gamma, righting_time_pois, righting_time_log)
# Log transforming is the best!
...
feeding_glm_plot <- glmmTMB(feed_bin ~ arm_bin*substrate + depth_m + (1|site), 
                       family = binomial(link = 'logit'),
                       data = feeding_data)
